...
// delimiter kind (mirroring the easyjson build's jlexer.Raw); json.Valid is the single source of
// truth for structural correctness, so both build tags accept and reject the same input.
...
// the caller validates the captured bytes via json.Valid. This mirrors jlexer.SkipRecursive in
// the easyjson build so both tags capture the same bytes.
